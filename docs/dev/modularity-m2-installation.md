...
PR 22 alert #8 identified a path-based read after `lstat` in the M2 test snapshot helper. A deterministic regression replaced the checked file before the read and demonstrated that the old helper accepted replacement bytes. The initial handle-based fix still performed `lstat` before `open`, which the exact CodeQL query flags independently of checks made afterward (follow-up alert #9). The shared `tests/helpers/filesystem-snapshot.mjs` now uses directory entries for traversal and opens regular files before any path check. It validates the opened handle, checks current path identity afterward, and obtains content and metadata through the handle. Both M1 and M2 fixtures use it. Regression tests enforce open-before-check ordering and cover a symlink replacement before opening, regular-file/symlink replacement immediately after opening, and replacement after handle validation. No suppression or test exclusion was added.
