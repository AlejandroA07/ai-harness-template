...
const userOnly = await readInvocationPolicy(path.join(root, 'skills'));
const estimate = (text) => Math.ceil(Buffer.byteLength(text, 'utf8') / 4);

const rows = [];
for (const skill of skills) {
  const metadata = `${skill.name}: ${skill.description}`;
  const resourceFiles = [];
  async function walk(directory) {
    for (const entry of await fs.readdir(directory, { withFileTypes: true })) {
      ...
      else resourceFiles.push(child);
    }
  }
  await walk(skill.directory);
  let resourceBytes = 0;
  for (const file of resourceFiles) resourceBytes += (await fs.stat(file)).size;
  rows.push(`| ${skill.name} | ${userOnly.has(skill.name) ? 'user-only' : 'model-invoked'} | ${estimate(metadata)} | ${estimate(skill.body)} | ${resourceFiles.length} / ${resourceBytes} |`);
}

const globalFiles = ['global/AGENTS.md', 'global/CLAUDE.md'];
const globalRows = [];
for (const relative of globalFiles) {
  const text = await fs.readFile(path.join(root, relative), 'utf8');
  globalRows.push(`| ${relative} | ${Buffer.byteLength(text, 'utf8')} | ${estimate(text)} |`);
}

const markdown = `# Token costs
...
${deniedClaudeBuiltInTools.map((tool) => `- \`${tool}\``).join('\n')}
