...
| **Fresh session** | Empty the context window and start from nothing; use `/clear` only where available. |
| **`/handoff`** | Write a portable markdown file and seed a session anywhere with it. |
| **Subagent** | Send the task to its own context window and get a report back.     |
| **Compact** | Use the current harness's compaction capability to seed the continuation with a summary. |
...
**2. Is the context irrelevant to what comes next?** Is everything in this session — the exploration, the decisions, the dead ends — disposable? If so, start a **fresh task or session**. Use `/clear` where the harness provides it; otherwise open a new task/session. It is the cheapest move on the board and the old task remains available for reference.
...
**5. Otherwise, compact.** Relevant context, same harness, same directory, and you need to stay in the loop — this is where the tree lands, and it lands here often. Use the harness's compaction control and state the next phase explicitly (for example, "keep what we need to QA this area") so the summary preserves the right material.
