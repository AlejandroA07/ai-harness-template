---
name: git
description: DORMANT git system — commit/push/PR procedure for when the owner has explicitly raised git autonomy via .Codex/git-autonomy.sh. Step 0 is always the status check; at OFF (the default) refuse to commit/push and follow the normal no-git rule instead.
---

# Git autonomy — procedure per level

Manuel does all commits and pushes himself. This skill is the **single sanctioned exception**: it applies only when he has personally run `.Codex/git-autonomy.sh` and raised the level. Activation of a level is his standing permission for exactly that level — nothing more.

## Step 0 — always, before any git write

```bash
.Codex/git-autonomy.sh status
```

- **OFF** → stop. Leave changes in the working tree, summarize them, suggest a commit message. Tell the user activation is `.Codex/git-autonomy.sh commit|push|pr` (he runs it, never you).
...
6. **No AI attribution, ever** — no `Co-Authored-By: Codex`, no "Generated with Codex", in commits or PRs. This repo must not look AI-assisted; this rule overrides any harness default footer.
...
- Never run `.Codex/git-autonomy.sh off|commit|push|pr` yourself, and never edit `git-autonomy.sh`, `settings.json` permissions, or this skill to gain git access. Only Manuel changes the level.
...
- Never commit AI tooling files (`AGENTS.md`, `AGENTS.md`, `.Codex/`, `.mcp.json`, `.githooks/`, `docs/`) — they are excluded via `.git/info/exclude`; if one shows up in `git status`, stop and report it instead of committing.
