...
# compgen instead of `ls glob...`: ls exits non-zero if ANY glob is unmatched.
if compgen -G "*.sln" >/dev/null || compgen -G "*.slnx" >/dev/null || compgen -G "*.csproj" >/dev/null; then
  cs_files=$(changed '*.cs')
  if [ -n "$cs_files" ]; then
    # Whole-solution format: `--include <list>` silently no-ops on larger lists
    # (observed with dotnet 10.0.201 + slnx), so format everything instead.
