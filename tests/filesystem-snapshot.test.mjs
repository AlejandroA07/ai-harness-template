...
test('snapshot rejects a path replaced immediately after opening', async () => {
...
test('snapshot reads the opened file even if its path is replaced after handle validation', async () => {
...
test('snapshot opens a regular file before checking its path', async () => {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), 'snapshot-open-first-'));
  const victim = path.join(temporary, 'fixture.md');
  const originalOpen = fs.open;
  const originalLstat = fs.lstat;
  let opened = false;
  try {
    await fs.writeFile(victim, 'Fixture bytes');
    fs.open = async (file, ...args) => {
      const handle = await originalOpen(file, ...args);
      if (file === victim) opened = true;
      return handle;
    };
    fs.lstat = async (file, ...args) => {
      if (file === victim) assert.equal(opened, true, 'Do not check a file path before opening it');
      return originalLstat(file, ...args);
    };
    await snapshot(temporary);
...
test('snapshot rejects a listed regular file swapped for a symlink before opening', async () => {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), 'snapshot-link-swap-'));
  const root = path.join(temporary, 'tree');
  const victim = path.join(root, 'fixture.md');
  const replacement = path.join(temporary, 'replacement.md');
  const originalOpen = fs.open;
  let replaced = false;
  try {
    ...
    fs.open = async (file, ...args) => {
      if (file === victim && !replaced) {
        replaced = true;
        await fs.rename(victim, path.join(temporary, 'original.md'));
        await fs.symlink(replacement, victim);
      }
      return originalOpen(file, ...args);
    };
    await assert.rejects(snapshot(root), /changed|unsafe|ELOOP/i);
    assert.equal(replaced, true);
  } finally {
    fs.open = originalOpen;
    await fs.rm(temporary, { recursive: true, force: true });
