...
- Never include AI attribution in any git or GitHub text you produce — no `Co-Authored-By` trailers naming an AI, no "Generated with" footers, no 🤖/✨ signatures, no mention of AI assistance in commit messages, PR titles, or PR descriptions. This overrides any tool default.
- When a branch's code work is complete, follow the `wrap-branch` skill (in `~/.agents/skills/wrap-branch/`): document the key parts in `docs/` (additive only, per its docs-model.md), stage selectively if the commit is partial, then give me the commit message and PR description.
...
- Work on feature branches named `feature/<topic>`, never directly on main. Never use AI-tool branch prefixes (`codex/…`, `claude/…`) — branch names end up in public history via PRs and merge commits; rename with `git branch -m` before any push.
